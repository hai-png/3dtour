   ...
   jQuery(function() {
    ...
    jQuery.each( elems, function( i, elem ) {
        ...
        jQuery(elem+' li').on( 'click', function(event) {
            ...
            // Check if this is a country dropdown item
            var countryButton = jQuery(this).closest('.dropdown').find('button');
            var countryButtonId = countryButton.attr('id') || '';
            var isCountryDropdown = countryButtonId === 'advanced_country' || 
                                    countryButtonId === 'sidebar-advanced_country' ||
                                    countryButtonId === 'shortcode-advanced_country' ||
                                    countryButtonId === 'mobile-advanced_country' ||
                                    // Treat county/state dropdowns that are labelled as Country as country filters too
                                    countryButtonId === 'county-state' ||
                                    countryButtonId === 'sidebar-county-state' ||
                                    countryButtonId === 'shortcode-county-state' ||
                                    countryButtonId === 'mobile-county-state';
            
            if (isCountryDropdown) {
                // Filter cities based on selected country
                var wpestateNormalizeCountry = function(v) {
                    return String(v || '')
                        .toLowerCase()
                        .trim()
                        .replace(/\s+/g, '-')
                ...
                // Some country dropdowns store the readable value in the text, not in data-value
                // Also normalize the visible text (pick) in case data-value is empty or different
                var selectedCountryRaw = value || pick || '';
                var selectedCountrySlug = wpestateNormalizeCountry(selectedCountryRaw);
                
                // Map country dropdown IDs to corresponding city dropdown IDs
                var cityIdMap = {
                    'advanced_country': 'advanced_city',
                    'sidebar-advanced_country': 'sidebar-advanced_city',
                    'shortcode-advanced_country': 'shortcode-advanced_city',
                    'mobile-advanced_country': 'mobile-advanced_city',
                    // When using county/state field as Country, still filter the same city dropdowns
                ...
                var targetCityId = cityIdMap[countryButtonId];
                if (targetCityId) {
                    var cityElement = jQuery('#' + targetCityId);

                    // Advanced search type 6 cities are a bootstrap-select <select>
                    if (cityElement.length > 0 && cityElement.is('select') && cityElement.hasClass('wpestatemultiselectselectpicker')) {
                        var filterActive = (selectedCountrySlug !== '' && selectedCountrySlug !== 'all');
                        var matchedCount = 0;

                        // First, try matching by data-parent-value (county/state) - this is the most common case
                        cityElement.find('option').each(function() {
                            var opt = jQuery(this);
                            var shouldKeep = true;
                            ...
                            if (filterActive) {
                                ...
                                // Try 1: Match by data-parent-value (county/state slug) - most common
                                var parentVal = opt.attr('data-parent-value') || '';
                                var parentValSlug = wpestateNormalizeCountry(parentVal);
                                if (parentValSlug !== '' && parentValSlug === selectedCountrySlug) {
                                    shouldKeep = true;
                                }
                                
                                // Try 2: Match by data-parent-value using original text (in case it's stored as name, not slug)
                                if (!shouldKeep && parentVal !== '') {
                                    var parentValNormalized = wpestateNormalizeCountry(parentVal);
                                    var selectedNormalized = wpestateNormalizeCountry(selectedCountryRaw);
                                    if (parentValNormalized === selectedNormalized || 
                                        parentVal.toLowerCase().indexOf(selectedCountryRaw.toLowerCase()) !== -1 ||
                                        selectedCountryRaw.toLowerCase().indexOf(parentVal.toLowerCase()) !== -1) {
                                        shouldKeep = true;
                                    }
                                }
                                
                                // Try 3: Match by data-parentcountry (actual country)
                                if (!shouldKeep) {
                                    var cityCountry = opt.attr('data-parentcountry') || '';
                                    var cityCountrySlug = wpestateNormalizeCountry(cityCountry);
                                    if (cityCountrySlug !== '' && cityCountrySlug === selectedCountrySlug) {
                                        shouldKeep = true;
                                    }
                                }
                                
                                // Try 4: Match by data-parentcounty attribute (alternative format)
                                if (!shouldKeep) {
                                    var cityCounty = opt.attr('data-parentcounty') || '';
                                    var cityCountySlug = wpestateNormalizeCountry(cityCounty);
                                    if (cityCountySlug !== '' && cityCountySlug === selectedCountrySlug) {
                                        shouldKeep = true;
                                    }
                                }
                            }

                            opt.prop('disabled', !shouldKeep);
                            opt.prop('hidden', !shouldKeep);
                            if (filterActive && shouldKeep) {
                                matchedCount++;
                            }
                        });
                        ...
                        cityElement.selectpicker('refresh');
                    } else {
                        // Fallback: regular dropdown list (<li>)
                        var cityMenu = cityElement.siblings('.dropdown-menu');
                        if (cityMenu.length === 0) {
                            cityMenu = cityElement.closest('.dropdown').find('.dropdown-menu').first();
                        }

                        var anyMatch = false;
                        var treatAsCounty = countryButtonId === 'county-state' ||
                                            countryButtonId === 'sidebar-county-state' ||
                                            countryButtonId === 'shortcode-county-state' ||
                                            countryButtonId === 'mobile-county-state';

                        cityMenu.find('li').each(function() {
                            var li = jQuery(this);

                            // Decide which attribute to use as parent identifier:
                            // - data-parentcountry when using real Country field
                            // - data-parentcounty when using County/State as “Country”
                            var parentAttr = treatAsCounty ? 'data-parentcounty' : 'data-parentcountry';
                            var cityParentRaw = li.attr(parentAttr) || '';

                            // Some datasets store multiple parents in a single attribute,
                            // e.g. "ethiopia,kenya". We need to treat this as a list.
                            var shouldShow = true;

                            if (selectedCountrySlug !== '' && selectedCountrySlug !== 'all') {
                                shouldShow = false;

                                // Split on comma and whitespace, normalize each token.
                                var rawTokens = cityParentRaw.split(',');
                                for (var i = 0; i < rawTokens.length; i++) {
                                    var token = wpestateNormalizeCountry(rawTokens[i]);
                                    if (token && token === selectedCountrySlug) {
                                        shouldShow = true;
                                        anyMatch = true;
                                        break;
                                    }
                        ...
                        // Safety net: if no city matched this country/county (maybe missing parent data),
                        // fall back to showing all cities so the dropdown is never “stuck” empty.
                        if (selectedCountrySlug !== '' && selectedCountrySlug !== 'all' && anyMatch === false) {
                            cityMenu.find('li').show();
                        }
                    }
                }
            }
...
function wpestate_advnced_filters_bars() {
    function initializeSelectPicker(element) {
        element.selectpicker({
            ...
            // Handle country to city filtering
            if (elementId && country_relations[elementId]) {
                var wpestateNormalizeCountry = function(v) {
                    return String(v || '')
                        .toLowerCase()
                        .trim()
                ...
                var selectedCountryValue = jQuery(this).find('option:selected').map(function() {
                    return jQuery(this).val();
                }).get();
                
                var cityElementId = country_relations[elementId];
                var cityElement = elememtParent.find('#' + cityElementId);
                
                if (cityElement.length > 0) {
                    var selectedCountrySlugs = selectedCountryValue
                        .map(wpestateNormalizeCountry)
                        .filter(function(v) { return v !== '' && v !== 'all'; });

                    // Selectpicker (advanced search type 6): disable/enable options + refresh (do NOT destroy/re-init)
                    if (cityElement.hasClass('wpestatemultiselectselectpicker')) {
                        cityElement.find('option').each(function() {
                            var opt = jQuery(this);
                            ...
                            if (selectedCountrySlugs.length > 0) {
                                ...
                                // Try 1: Match by data-parent-value (county/state slug) - most common case
                                var parentVal = opt.attr('data-parent-value') || '';
                                var parentValSlug = wpestateNormalizeCountry(parentVal);
                                for (var i = 0; i < selectedCountrySlugs.length; i++) {
                                    if (parentValSlug !== '' && parentValSlug === selectedCountrySlugs[i]) {
                                        shouldEnable = true;
                                        break;
                                    }
                                    // Also try partial match (in case stored as name vs slug)
                                    if (parentVal !== '' && (
                                        parentVal.toLowerCase().indexOf(selectedCountrySlugs[i]) !== -1 ||
                                        selectedCountrySlugs[i].indexOf(parentValSlug) !== -1
                                    )) {
                                        shouldEnable = true;
                                        break;
                                    }
                                ...
                                // Try 2: Match by data-parentcountry (actual country)
                                if (!shouldEnable) {
                                    var cityCountry = opt.attr('data-parentcountry') || '';
                                    var cityCountrySlug = wpestateNormalizeCountry(cityCountry);
                                    for (var i = 0; i < selectedCountrySlugs.length; i++) {
                                        if (cityCountrySlug !== '' && cityCountrySlug === selectedCountrySlugs[i]) {
                                            shouldEnable = true;
                                            break;
                                        }
                                    }
                                ...
                                // Try 3: Match by data-parentcounty (alternative format)
                                if (!shouldEnable) {
                                    var cityCounty = opt.attr('data-parentcounty') || '';
                                    var cityCountySlug = wpestateNormalizeCountry(cityCounty);
                                    for (var i = 0; i < selectedCountrySlugs.length; i++) {
                                        if (cityCountySlug !== '' && cityCountySlug === selectedCountrySlugs[i]) {
                                            shouldEnable = true;
                                            break;
                                        }
                                    }
                            ...
                            opt.prop('disabled', !shouldEnable);
                            opt.prop('hidden', !shouldEnable);
                        });

                        // When a country is selected, clear city selection to avoid "stuck / stacked" UI
                        if (selectedCountrySlugs.length > 0) {
                            cityElement.val([]);
                        }
                        ...
                        // Regular dropdown: show/hide <li> items
                        var cityMenu = cityElement.siblings('.dropdown-menu');
                        if (cityMenu.length === 0) {
                            cityMenu = cityElement.closest('.dropdown').find('.dropdown-menu').first();
                        }
                        ...
                        cityMenu.find('li').each(function() {
                            var li = jQuery(this);
                            var cityCountry = li.attr('data-parentcountry') || '';
                            var cityCountrySlug = wpestateNormalizeCountry(cityCountry);

                            var shouldShow = true;
                            if (selectedCountrySlugs.length > 0) {
                                shouldShow = (cityCountrySlug !== '' && selectedCountrySlugs.indexOf(cityCountrySlug) !== -1);
                            }
